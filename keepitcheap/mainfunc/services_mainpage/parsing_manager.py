...
from mainfunc.models import Items, Categories
from mainfunc.services_mainpage.parser import get_parse_data
...
class CompareParseWithModel:
    ...
    def clean_pag_cache(self, item_id: int) -> None:
        """Слишком частое обращение к кэшу будет, если много товаров обновлено на одной странице
        пока что хз как решить """
        page_num = math.ceil(item_id / settings.PAGINATION_SIZE)  # Выясняем на какой странице предмет
        if cache.get(f'{page_num}_{settings.ITEMS_CACHE_NAME}'):  # если страница была кэширована, то чистим её
            cache.delete(f'{page_num}_{settings.ITEMS_CACHE_NAME}')
...
class ParsingFunctional(CompareParseWithModel):
    ...
    def create_item_notification_info_for_telegram(self, user, item, message):
        ...
        if user_telegram_id:  # если юзер есть в боте, то все норм
            ...
            """Сильная привязанность к порядку элементов, в этом же порядке будут передаваться в тг бот"""
